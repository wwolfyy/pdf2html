...
from helpers.utils_other import render_bboxes_2_with_image, parse_layout, parse_texts_area, \
...
# %% define functions to deal with idiosyncracy

# function to deal with idiosyncracy (thesis)
def ocr_rules_leg_trend(
    block,
    tag
):
    pass
...
PP_font_path = '/NotoSansKR-Regular.otf'
text_parser = 'pymupdf'  # or naver
# doc path
...
@app.post("/pdf")
async def main(folder: str = Form(...)):
    ...
    # Loop over each PDF file and read its contents using PyMuPDF
    for pdf_file in pdf_files:
        with open(pdf_file, 'rb') as f:
            ...
            if rotation != 0:
                ...
                raise Exception("Only porttrait orientation is supported for now. 현재는 세로 형식의 문서만 지원합니다.")

            # Do something with the PDF document, such as extracting text or images
            processed_doc = process_doc(doc)

        # Return the processed PDF document as a response
        yield {"status": "success", "document": processed_doc}

    # Return a final response indicating that all documents have been processed
    return {"status": "done"}


def process_doc_trend(doc: fitz.Document):
    ...
    page_dict = parse_texts(parser=text_parser, pymupdf_page_object=page, image_byte_for_ocr=None)
...
# Set the folder path to read PDF files from
folder_path = "/path/to/folder"

# Send a POST request to the API endpoint with the folder path as a form field
response = requests.post(url, data={"folder": folder_path})

# Check the response status code
if response.status_code == 200:
    print("PDF files read successfully")
else:
...
# Set the folder path to save the processed PDF documents to
output_folder = "/path/to/output/folder"

# Send a POST request to the API endpoint with the folder path as a form field
response = requests.post(url, data={"folder": folder_path}, stream=True)

# Iterate over the response to get each processed PDF document as it becomes available
for chunk in response.iter_content(chunk_size=None):
    # Parse the chunk as a JSON object
    data = json.loads(chunk)

    # Check if the status is "success" and the document is not empty
    if data["status"] == "success" and data["document"]:
        # Get the document object from the response data
        doc = data["document"]

        # Get the filename of the processed PDF document
        filename = os.path.basename(doc.name)

        # Save the processed PDF document to the output folder
        doc.save(os.path.join(output_folder, filename))

# Print a message indicating that all documents have been processed
print("All PDF documents processed")